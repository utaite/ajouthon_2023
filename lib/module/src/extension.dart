...
extension RxNullableEx<T> on Rx<T>? {
  Widget rx(Widget Function(Rx<T> rx) builder, {Widget? onEmpty}) => this is Rx<T> ? ObxValue<Rx<T>>(builder, this!) : onEmpty ?? const SizedBox.shrink();
}

extension GetXRxNullableEx<T> on getx.Rx<T>? {
  Widget rx(Widget Function(getx.Rx<T> rx) builder, {Widget? onEmpty}) =>
      this is getx.Rx<T> ? ObxValue<getx.Rx<T>>(builder, this!) : onEmpty ?? const SizedBox.shrink();
}

extension RxListNullableEx<T> on RxList<T>? {
  Widget rx(Widget Function(RxList<T> rx) builder, {Widget? onEmpty}) =>
      this is RxList<T> ? ObxValue<RxList<T>>(builder, this!) : onEmpty ?? const SizedBox.shrink();
}

extension GetXRxListNullableEx<T> on getx.RxList<T>? {
  Widget rx(Widget Function(getx.RxList<T> rx) builder, {Widget? onEmpty}) =>
      this is getx.RxList<T> ? ObxValue<getx.RxList<T>>(builder, this!) : onEmpty ?? const SizedBox.shrink();
}

extension RxMapNullableEx<K, V> on RxMap<K, V>? {
  Widget rx(Widget Function(RxMap<K, V> rx) builder, {Widget? onEmpty}) =>
      this is RxMap<K, V> ? ObxValue<RxMap<K, V>>(builder, this!) : onEmpty ?? const SizedBox.shrink();
}

extension OffsetOptionalEx on Offset? {
  Offset get elvis => this ?? Offset.zero;
}
...
extension ValueNullableEx<T> on Value<T>? {
  Widget rx(
    ...
    Widget? onEmpty,
      ...
      this?.rx(widget, onError: onError, onLoading: onLoading, onEmpty: onEmpty) ?? const SizedBox.shrink();
}
...
extension WidgetEx on Widget {
  Future<T?> showDialog<T>({Bindings? binding, bool barrierDismissible = false, Iterable<dynamic>? arguments}) async {
